...
class ROIHeads(torch.nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.vp_bins                  = cfg.VP_BINS
        self.vp_weight_loss           = cfg.VP_WEIGHT_LOSS
        self.weights_height           = cfg.WEIGHTS_HEIGHT
        # fmt: on
...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.pooler = ROIPooler(
            ...
            canonical_level=int((cfg.MODEL.RESNETS.OUT_FEATURES[-1].split('res'))[1])
        )
    ...
    def _build_res5_block(self, cfg):
        # fmt: off
        stage_channel_factor = 2 ** (int((cfg.MODEL.RESNETS.OUT_FEATURES[-1].split('res'))[1])-1)  # res5 is 8x res2
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        pred_class_logits, pred_proposal_deltas, viewpoint_logits, viewpoint_residuals, height_logits = self.box_predictor(feature_pooled)
...
class StandardROIHeads(ROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        pred_class_logits, pred_proposal_deltas, viewpoint_logits, viewpoint_residuals, height_logits = self.box_predictor(box_features)
        del box_features
