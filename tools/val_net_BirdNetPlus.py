...
def parse_args():
    ...
    parser.add_argument(
        '--ann_val',
        help="Validation file with the annotations in COCO format previously generated by the training script, without extension",
        default='validation_annotations_kitti_carpedcyc_RDHCVPr12', type=str)
    parser.add_argument(
        '--write', help="Write results in KITTI format", default=True, action="store_true")
    parser.add_argument(
        '--img2show', help="Show a fixed number of images, 0 to eliminate the visualization", default=3000, type=int)
    parser.add_argument(
        '--save_img', help="Save images showed", default=True, action="store_true")
    parser.add_argument(
        '--eval_chkp',
        help="Starting from the second half of the checkpoints, the rest will be evaluated with a certain interval specified here, 1 to evaluate all of them",
        default=1, type=int)
    parser.add_argument(
        '--force_test', help="Name of the checkpoint to extract annotations or evaluate, empty disable this option",
        default='', type=str)
    parser.add_argument(
        '--score', help="Limitation for lower scores", default=0.01, type=float)
    parser.add_argument(
        '--nms', help="NMS IoU for the overlapping obstacles per class", default=0.5, type=float)
    parser.add_argument(
        '--kitti_root', help="Path of the KITTI dataset",
        default='/home/ri-1080/work2/birdnet2/datasets/kitti/object/training', type=str)
    # if len(sys.argv) == 1:
    #     parser.print_help()
    #     sys.exit(1)
...
idclass = {0: 'Car', 1: 'Van', 2: 'Truck', 3: 'Pedestrian', 4: 'Person_sitting', 5: 'Cyclist', 6: 'Tram', 7: 'Misc',
           8: 'DontCare'}
...
def prepare_for_coco_detection_KITTI(instance, output_folder, filename, write, kitti_calib_path, nclass, vp, bins,
                                     vp_res, hwrot, height_training):
    ...
    if vp_res:
        alpha = np.array([rad for rad in instance.get('viewpoint_residual')]) if vp else np.ones((labels.shape)) * (
            -10.00)
    else:
        alpha = np.array([getfrombins(cl, bins) for cl in instance.get('viewpoint')]) if vp else np.ones(
            (labels.shape)) * (-10.00)

    h = np.array([[h, g] for h, g in instance.get('height')]) if height_training else np.array(
        [-1, -1000] * labels.shape)
    ...
    # Calibration for 3D
    calib_file = os.path.join(kitti_calib_path, filename.split('/')[-1].split('.')[0] + '.txt')
    ...
    im_ann_obj = []
    if write:
        file_ann = open(os.path.join(output_folder, filename.split('/')[-1].split('.')[0] + '.txt'), 'w+')
...
def main(config_file, ann_val, write, img2show, save_img, eval_chkp, force_test, score_thresh, nms_thresh, kitti_root):
    ...
    for checkpoint, eval_folder in zip(toeval, f_eval):
        ...
        for image_id, d in enumerate(val_bv_dicts):  # filename.split('/')[-1].split('.')[0]
            c += 1
            file = os.path.join(ann_outdir, d["file_name"].split('/')[-1].split('.')[0] + '.txt')
            ...
            if not os.path.exists(file) or write:
                ...
                list_anns, obj_anns, instances = prepare_for_coco_detection_KITTI(outputs["instances"].to("cpu"),
                                                                                  ann_outdir, d["file_name"], write,
                                                                                  kitti_calib_path, nclasses,
                                                                                  cfg.VIEWPOINT, cfg.VP_BINS,
                                                                                  cfg.VIEWPOINT_RESIDUAL,
                                                                                  cfg.ROTATED_BOX_TRAINING,
                                                                                  cfg.HEIGHT_TRAINING)
            ...
            if c in sample_idx:
                ...
                kitti_im = cv2.imread(os.path.join(kitti_im_path, d["file_name"].split('/')[-1]))
                calib_file = os.path.join(kitti_calib_path, d["file_name"].split('/')[-1].split('.')[0] + '.txt')
                # Show obstacles
                for i, obj in enumerate(obj_anns):
                    kitti_im, im, _ = _draw_projection_obstacle_to_cam(obj, calib_file, bvres, only_front, True,
                                                                       kitti_im, im, is_kitti_ann=is_kitti_ann)
                ...
                if save_img:
                    ...
                    cv2.imwrite(os.path.join(im_outdir, '3D_' + d["file_name"].split('/')[-1]), kitti_im)
                    cv2.imwrite(os.path.join(im_outdir, 'BEV_' + d["file_name"].split('/')[-1]), im)
                cv2.waitKey(0)
...
if __name__ == '__main__':
    ...
    main(args.config_file, args.ann_val, args.write, args.img2show, args.save_img, args.eval_chkp, args.force_test,
         args.score, args.nms, args.kitti_root)
